...
class Square():
    def __init__(self, screen_w = SCREEN_W, screen_h = SCREEN_H, width = 100, height = 100, color = (50,200,50)):
        self.width = width
        self.height = height
        self.start_x = take_random_coordinate(screen_w, width, cherepashka.xcor())
        self.start_y = take_random_coordinate(screen_h, height, cherepashka.ycor())
        self.color = color

win_square = Square(SCREEN_W, SCREEN_H, random.randint(20,100), random.randint(20,100))

defeat_zones: list[Square] = []
for i in range(10):
    defeat_zones.append(Square(width= random.randint(20,100), height= random.randint(20,100), color=(200,50,50)))

def restricted_area_check():
    compensator = 20
...
def make_move(angle):
    ...
    if cherepashka_step == 2:
        cherepashka_step = 0
        defeat_zones.append(Square(width= random.randint(20,100), height= random.randint(20,100), color=(50,50,200)))
        make_square(defeat_zones[-1], finish_turtle)
...
def make_square(square: Square, trtl: turtle):
    trtl.speed(0)
...
make_square(win_square, finish_turtle)
for square in defeat_zones:
    make_square(square, finish_turtle)
